...
# We have to fetch V from the server
fV = gf.Fetch(V, host="http://amb25.stccmop.org:6767")
...
aH = gf.Accumulate("column", "column+%i-b" % fV.getResult().Card(0), "0", 0, ordH, -1)

# Restrict the horizontal grid to the region of interests
rH = gf.Restrict("(%s<x) & (x<%s) & (%s<y) & (y<%s)" % bounds, 0, aH)

fetch_rH = gf.Fetch(rH, host="http://amb25.stccmop.org:6767")
...
# Package up the expression so far and send it off to  
# the server for evaluation.
fetch = gf.Fetch(B, host="http://amb25.stccmop.org:6767")
...
freeW = gf.Apply("zfree=max(z-surf,-h);u=-99*(salt=-99)+u*(salt>-99);v=-99*(salt=-99)+v*(salt>-99)", 0, fetch)
...
joinA = gf.Aggregate(join, both, fetch_rH, 0, rW, 0)
joinA.getResult()
...
sT = gf.Scan(context, "T")

T = gf.Fetch(sT, host="http://amb25.stccmop.org:6767")
