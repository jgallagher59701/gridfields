...
BindOp::BindOp(Array *arr, Dim_t k, GridFieldOperator *op)
 : UnaryGridFieldOperator(op), /* Unused. jhrg 4/4/14 array(arr),*/ attr(arr->getName()), type(arr->type),
   reader(NULL), _k(k)
{cout<<"ahoy!"<<endl;
//  this->cleanup = false;
}
...
BindOp::BindOp(Array *arr, ArrayReader *rdr, Dim_t k, GridFieldOperator *op)
 : UnaryGridFieldOperator(op), /* Unused array(arr),*/ attr(arr->getName()), type(arr->type),
   reader(rdr), _k(k)
{
//  this->cleanup = false;
}
