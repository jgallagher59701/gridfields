...
int RTREE_QUAL::PickBranch(Rect* a_rect, Node* a_node)
{
  ...
  {
    Rect* curRect = &a_node->m_branch[i].m_rect;
...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(Rect* a_rectA, Rect* a_rectB)
{
  ...
  {
    newRect.m_min[i] = Min(a_rectA->m_min[i], a_rectB->m_min[i]);
    newRect.m_max[i] = Max(a_rectA->m_max[i], a_rectB->m_max[i]);
  }
...
ELEMTYPEREAL RTREE_QUAL::RectSphericalVolume(Rect* a_rect)
{
  ...
  for(int i=0; i < NUMDIMS; ++i) 
  {
    ELEMTYPEREAL halfExtent = ((ELEMTYPEREAL)a_rect->m_max[i] - (ELEMTYPEREAL)a_rect->m_min[i]) * 0.5f;
...
void RTREE_QUAL::GetBranches(Node* a_node, Branch* a_branch, PartitionVars* a_parVars)
{
  ...
  {
    a_parVars->m_branchBuf[i] = a_node->m_branch[i];
  }
  ...
  {
    a_parVars->m_coverSplit = CombineRect(&a_parVars->m_coverSplit, &a_parVars->m_branchBuf[i].m_rect);
  }
...
void RTREE_QUAL::ChoosePartition(PartitionVars* a_parVars, int a_minFill)
{
       ...
       && (a_parVars->m_count[1] < (a_parVars->m_total - a_parVars->m_minFill)))
  {
    ...
    for(int i=0; i<a_parVars->m_total; ++i)
    {
      if(!a_parVars->m_taken[i])
      {
...
void RTREE_QUAL::LoadNodes(Node* a_nodeA, Node* a_nodeB, PartitionVars* a_parVars)
{
  ...
  for(int i=0; i < a_parVars->m_total; ++i)
  {
    ASSERT(a_parVars->m_partition[i] == 0 || a_parVars->m_partition[i] == 1);
    
    if(a_parVars->m_partition[i] == 0)
    {
      AddBranch(&a_parVars->m_branchBuf[i], a_nodeA, NULL);
    }
    else if(a_parVars->m_partition[i] == 1)
    {
      AddBranch(&a_parVars->m_branchBuf[i], a_nodeB, NULL);
    }
  }
}
...
void RTREE_QUAL::InitParVars(PartitionVars* a_parVars, int a_maxRects, int a_minFill)
{
  ...
  for(int i=0; i < a_maxRects; ++i)
  {
    a_parVars->m_taken[i] = false;
    a_parVars->m_partition[i] = -1;
  }
}
...
void RTREE_QUAL::PickSeeds(PartitionVars* a_parVars)
{
  ...
  for(int i=0; i<a_parVars->m_total; ++i)
  {
    area[i] = CalcRectVolume(&a_parVars->m_branchBuf[i].m_rect);
  }
...
bool RTREE_QUAL::RemoveRect(Rect* a_rect, const DATATYPE& a_id, Node** a_root)
{
  ...
  if(!RemoveRectRec(a_rect, a_id, *a_root, &reInsertList))
  {
    ...
    while(reInsertList)
    {
      ...
      {
        InsertRect(&(tempNode->m_branch[i].m_rect),
                   tempNode->m_branch[i].m_data,
                   a_root,
                   tempNode->m_level);
      }
...
bool RTREE_QUAL::RemoveRectRec(Rect* a_rect, const DATATYPE& a_id, Node* a_node, ListNode** a_listNode)
{
  ...
  if(a_node->IsInternalNode())  // not a leaf node
  {
    for(int i = 0; i < a_node->m_count; ++i)
    {
      ...
      {
        if(!RemoveRectRec(a_rect, a_id, a_node->m_branch[i].m_child, a_listNode))
        {
          if(a_node->m_branch[i].m_child->m_count >= MINNODES)
          {
            // child removed, just resize parent rect
            a_node->m_branch[i].m_rect = NodeCover(a_node->m_branch[i].m_child);
          }
          else
          {
            // child removed, not enough entries in node, eliminate node
            ReInsert(a_node->m_branch[i].m_child, a_listNode);
            DisconnectBranch(a_node, i); // Must return after this call as count has changed
          }
          return false;
        }
      }
  ...
  else // A leaf node
  {
    for(int i = 0; i < a_node->m_count; ++i)
    {
      if(a_node->m_branch[i].m_child == (Node*)a_id)
      {
        DisconnectBranch(a_node, i); // Must return after this call as count has changed
        return false;
      }
...
bool RTREE_QUAL::Overlap(Rect* a_rectA, Rect* a_rectB)
{
  ...
  for(int i=0; i < NUMDIMS; ++i)
  {
    if (a_rectA->m_min[i] > a_rectB->m_max[i] ||
        a_rectB->m_min[i] > a_rectA->m_max[i])
    {
      return false;
    }
  }
...
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, bool a_resultCallback(DATATYPE a_data, void* a_context), void* a_context)
{
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    for(int i=0; i < a_node->m_count; ++i)
    {
      if(Overlap(a_rect, &a_node->m_branch[i].m_rect))
      {
        if(!Search(a_node->m_branch[i].m_child, a_rect, a_foundCount, a_resultCallback, a_context))
        {
