...
NetCDFOp::NetCDFOp(string fn, GridFieldOperator *op)  
 : UnaryGridFieldOperator(op), offset(0), filename(fn) {
   GF = NULL;
   //cleanup = false;
}

NetCDFOp::NetCDFOp(string fn, long off, GridFieldOperator *op)  
...
void NetCDFOp::Execute() {
  this->PrepareForExecution();
  this->WriteNetCDF(this->GF, this->filename, this->offset);
  this->Result = this->GF;
}

void NetCDFOp::WriteNetCDF(vector<GridField *> &dims, GridField *cross) {
   ...
   for (int i=0; i<dims.size(); i++) {
     ...
     for (int j=0; j<gf->getarity(); j++) {
       ...
       size = (long) a->size;
       NetCDFOp::putData(a, ncdf, &size, &(d[i]), 1); 
     }
   }
   ...
   for (int j=0; j<cross->getarity(); j++) {
     ...
     NetCDFOp::putData(a, ncdf, counts, d, dims.size()); 
   }
...
NcVar *NetCDFOp::putData(Array *a, NcFile *ncdf, long *counts, NcDim **d, int dimcount) {
  Type t = a->type;
  NcVar *var = NULL;
  UnTypedPtr p;
...
*/
void NetCDFOp::NetCDF(GridField *GF, string filename, long offset) {
  /*
...
void NetCDFOp::writeGrid(Grid *G, ofstream &f) {
...
void NetCDFOp::writeGridField(GridField *GF, ofstream &f) {
...
void NetCDFOp::writeDataset(const Dataset &ds, ofstream &f) {
   int arity = ds.Arity();
   ...
   for (int i=0; i<arity; ++i) {
     ...
     aw.Write(ds, string(a->name));
   }
}


void NetCDFOp::writeCellArray(AbstractCellArray *ca, ofstream &f) {
   int size = ca->getsize();
...
void NetCDFOp::writeName(string name, ofstream &f) {
   int s = name.size();
   f.write((char *) &s, sizeof(int));
